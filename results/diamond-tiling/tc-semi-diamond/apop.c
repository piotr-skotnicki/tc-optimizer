...
/* ./tc ../examples/pluto/apop.scop.c --semi-diamond-tiling --omp-for-codegen --iterative-tc --inline --debug -b 128 --drop-bounds */
#define min(x,y)    ((x) < (y) ? (x) : (y))
#define max(x,y)    ((x) > (y) ? (x) : (y))
#define floord(n,d) (((n)<0) ? -((-(n)+(d)-1)/(d)) : (n)/(d))
#pragma scop
if (ns >= 2) {
  for (int ii0 = 0; ii0 <= floord(nt - 1, 128); ii0 += 1) {
    #pragma omp parallel for
    for (int ii1 = 0; ii1 < floord(ns - 2, 128); ii1 += 1) {
      for (int i0 = 128 * ii0; i0 <= min(min(nt - 1, 128 * ii0 + 62), ns + 128 * ii0 - 128 * ii1 - 130); i0 += 1) {
        for (int i1 = -128 * ii0 + 128 * ii1 + i0 + 129; i1 <= min(ns - 1, 128 * ii0 + 128 * ii1 - i0 + 254); i1 += 1) {
          F[(i0 + 1) % 2][i1] = max(((C[0][i1] * F[i0 % 2][i1 - 1]) + (C[1][i1] * F[i0 % 2][i1])) + (C[2][i1] * F[i0 % 2][i1 + 1]), E - ((i1) * dS));
        }
      }
    }
    for (int k = 1; k <= min(2, -ii0 + (nt + 126) / 128 + 1); k += 1) {
      for (int i0 = max(128 * ii0, 128 * ii0 + 128 * k - 255); i0 <= min(min(min(nt - 1, 128 * ii0 + 127), ns + 128 * ii0 + 127 * k - 129), 128 * ii0 + 64 * k + 62); i0 += 1) {
        for (int i1 = max(1, -128 * ii0 - 127 * k + i0 + 128); i1 <= min(min(ns - 1, 128 * ii0 - i0 + 254), -128 * ii0 - 128 * k + i0 + 256); i1 += 1) {
          F[(i0 + 1) % 2][i1] = max(((C[0][i1] * F[i0 % 2][i1 - 1]) + (C[1][i1] * F[i0 % 2][i1])) + (C[2][i1] * F[i0 % 2][i1 + 1]), E - ((i1) * dS));
        }
      }
      if (nt + 63 >= 128 * ii0 + 64 * k) {
        #pragma omp parallel for
        for (int ii1 = 1; ii1 < min(-ii0 + floord(nt + ns - 1, 128), (ns + 64 * k - 1) / 128); ii1 += 1) {
          if (k == 2) {
            for (int i0 = max(128 * ii0 + 64, -ns + 128 * ii0 + 128 * ii1 + 128); i0 <= min(nt - 1, 128 * ii0 + 127); i0 += 1) {
              for (int i1 = 128 * ii0 + 128 * ii1 - i0 + 127; i1 <= min(ns - 1, -128 * ii0 + 128 * ii1 + i0); i1 += 1) {
                F[(i0 + 1) % 2][i1] = max(((C[0][i1] * F[i0 % 2][i1 - 1]) + (C[1][i1] * F[i0 % 2][i1])) + (C[2][i1] * F[i0 % 2][i1 + 1]), E - ((i1) * dS));
              }
            }
          } else {
