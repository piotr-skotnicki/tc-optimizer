...
/* ./tc ../examples/polybench/lu.scop.c --correction-tiling --isl-wave-scheduling --omp-for-codegen --iterative-tc --debug -b 16 --align */
...
for (register int k0 = 0; k0 <= floord(3 * _PB_N - 4, 16); k0 += 1) {
  ...
  for (register int k1 = max(0, floord(-_PB_N + 16 * k0, 32) + 1); k1 <= min((_PB_N - 1) / 16, k0 / 2); k1 += 1) {
    for (register int k2 = -k1 + (k0 + k1 + 1) / 2; k2 < k1; k2 += 1) {
      if (k1 + 2 * k2 >= k0 + 1) {
    ...
    for (register int k2 = max(k1, k0 - 2 * k1); k2 <= min(k0 - k1, (_PB_N - 1) / 16); k2 += 1) {
      if (k0 == 0 && k1 == 0 && k2 == 0) {
        for (register int i0 = 1; i0 <= min(15, _PB_N - 1); i0 += 1) {
          A[i0][0] /= A[0][0];
          for (register int i2 = 1; i2 < i0; i2 += 1) {
            A[i0][i2] -= (A[i0][0] * A[0][i2]);
          }
        }
      } else if (3 * k1 >= k0 + 1 && k2 == k1) {
      ...
      for (register int i0 = max(16 * k1, 16 * k0 - 16 * k1 - 16 * k2 + 1); i0 <= min(_PB_N - 1, 16 * k1 + 15); i0 += 1) {
        if (k0 + 1 >= 3 * k1 && k2 == k1) {
          for (register int i2 = max(16 * k1, 16 * k0 - 32 * k1 + 1); i2 < i0; i2 += 1) {
            for (register int i4 = max(16 * k1, 16 * k0 - 32 * k1 + 1); i4 <= min(16 * k0 - 32 * k1 + 16, i2 - 1); i4 += 1) {
              A[i0][i2] -= (A[i0][i4] * A[i4][i2]);
            }
            if (16 * k0 + 16 >= 32 * k1 + i2) {
              A[i0][i2] /= A[i2][i2];
            }
          }
        }
  ...
  #pragma omp parallel for
  for (register int k1 = k0 / 2 + 1; k1 <= min(k0, (_PB_N - 1) / 16); k1 += 1) {
    for (register int k2 = -k1 + (k0 + k1 + 1) / 2; k2 <= k0 - k1; k2 += 1) {
      if (k1 + 2 * k2 >= k0 + 1) {
        for (register int i0 = 16 * k1; i0 <= min(_PB_N - 1, 16 * k1 + 15); i0 += 1) {
          for (register int i2 = 16 * k2; i2 <= 16 * k2 + 15; i2 += 1) {
            for (register int i4 = 16 * k0 - 16 * k1 - 16 * k2; i4 <= 16 * k0 - 16 * k1 - 16 * k2 + 15; i4 += 1) {
