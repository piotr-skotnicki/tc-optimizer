...
if ((_PB_M >= 1) && (_PB_N >= 1)) {
  if (_PB_M >= 2) {
    for (t2=0;t2<=floord(_PB_N-1,64);t2++) {
      for (t3=0;t3<=floord(_PB_M-2,64);t3++) {
        for (t4=t3;t4<=floord(_PB_M-1,64);t4++) {
          for (t5=64*t3;t5<=min(min(_PB_M-2,64*t3+63),64*t4+62);t5++) {
            for (t6=max(64*t4,t5+1);t6<=min(_PB_M-1,64*t4+63);t6++) {
              lbv=64*t2;
...
#pragma vector always
  ...
  for (t2=0;t2<=floord(_PB_M-1,64);t2++) {
    for (t3=0;t3<=floord(_PB_N-1,64);t3++) {
      for (t4=64*t2;t4<=min(_PB_M-1,64*t2+63);t4++) {
        lbv=64*t3;
        ubv=min(_PB_N-1,64*t3+63);
#pragma ivdep
#pragma vector always
