...
/* Start of CLooG code */
if (_PB_N >= 2) {
  for (t2=ceild(-_PB_N-61,64);t2<=0;t2++) {
    for (t4=max(0,-t2-1);t4<=floord(_PB_N-1,64);t4++) {
      if ((t2 <= floord(-_PB_N+2,64)) && (t4 >= ceild(_PB_N-63,64))) {
      ...
      if ((t2 == -t4-1) && (t2 >= ceild(-_PB_N,64))) {
        table[(-64*t2-2)][(-64*t2-1)] = max_score(table[(-64*t2-2)][(-64*t2-1)], table[(-64*t2-2)][(-64*t2-1)-1]);;
        table[(-64*t2-2)][(-64*t2-1)] = max_score(table[(-64*t2-2)][(-64*t2-1)], table[(-64*t2-2)+1][(-64*t2-1)]);;
        table[(-64*t2-2)][(-64*t2-1)] = max_score(table[(-64*t2-2)][(-64*t2-1)], table[(-64*t2-2)+1][(-64*t2-1)-1]);;
      }
      for (t5=max(max(64*t2,-_PB_N+3),-64*t4-61);t5<=min(min(0,64*t2+63),-64*t4+1);t5++) {
        ...
        for (t7=-t5+2;t7<=min(_PB_N-1,64*t4+63);t7++) {
      ...
      for (t5=max(64*t2,-64*t4+2);t5<=min(0,64*t2+63);t5++) {
        for (t7=64*t4;t7<=min(_PB_N-1,64*t4+63);t7++) {
          table[-t5][t7] = max_score(table[-t5][t7], table[-t5][t7-1]);;
          table[-t5][t7] = max_score(table[-t5][t7], table[-t5+1][t7]);;
          table[-t5][t7] = max_score(table[-t5][t7], table[-t5+1][t7-1]+match(seq[-t5], seq[t7]));;
          for (t9=-t5+1;t9<=t7-1;t9++) {
