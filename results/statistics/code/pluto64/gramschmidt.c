...
/* Start of CLooG code */
if (_PB_N >= 1) {
  for (t2=0;t2<=floord(_PB_N-2,64);t2++) {
    for (t4=t2;t4<=floord(_PB_N-1,64);t4++) {
      for (t5=64*t2;t5<=min(min(_PB_N-2,64*t2+63),64*t4+62);t5++) {
        lbv=max(64*t4,t5+1);
        ubv=min(_PB_N-1,64*t4+63);
#pragma ivdep
#pragma vector always
  ...
  for (t2=0;t2<=_PB_N-1;t2++) {
    ...
    for (t4=0;t4<=floord(_PB_M-1,64);t4++) {
      lbv=64*t4;
      ubv=min(_PB_M-1,64*t4+63);
#pragma ivdep
#pragma vector always
    ...
    if ((_PB_M >= 1) && (t2 <= _PB_N-2)) {
      for (t4=ceild(t2-62,64);t4<=floord(_PB_N-1,64);t4++) {
        for (t6=0;t6<=floord(_PB_M-1,64);t6++) {
          for (t7=max(64*t4,t2+1);t7<=min(_PB_N-1,64*t4+63);t7++) {
            for (t9=64*t6;t9<=min(_PB_M-1,64*t6+63);t9++) {
