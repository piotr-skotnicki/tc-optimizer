...
if ((_PB_N >= 3) && (_PB_TSTEPS >= 1)) {
  for (t2=1;t2<=_PB_TSTEPS;t2++) {
    for (t4=0;t4<=floord(_PB_N-2,64);t4++) {
      lbv=max(1,64*t4);
      ubv=min(_PB_N-2,64*t4+63);
#pragma ivdep
...
#pragma vector always
      ...
      for (t6=0;t6<=floord(_PB_N-2,64);t6++) {
        for (t7=max(1,64*t4);t7<=min(_PB_N-2,64*t4+63);t7++) {
          for (t9=max(1,64*t6);t9<=min(_PB_N-2,64*t6+63);t9++) {
...
#pragma vector always
      ...
      for (t6=ceild(-_PB_N-61,64);t6<=-1;t6++) {
        for (t7=max(1,64*t4);t7<=min(_PB_N-2,64*t4+63);t7++) {
          for (t9=max(64*t6,-_PB_N+2);t9<=64*t6+63;t9++) {
