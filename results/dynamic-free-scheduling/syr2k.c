...
#define S2_I(i,k,j) C[i][j] += A[j][k] * alpha*B[i][k] + B[j][k] * alpha*A[i][k];
...
int create_task(isl_point* point, void* user) {
  ...
  #pragma omp task
  {
    if (ii0 == 1 && ii1 >= 0 && ii2 >= 0 && ii3 >= 0) {
      ...
      for (int c1 = 128 * ii1; c1 <= min(N - 1, 128 * ii1 + 127); c1 += 1)
        for (int c2 = 128 * ii2; c2 <= min(N - 1, 128 * ii2 + 127); c2 += 1)
          S1(c1, c2);
  }
  return 0;
}
...
#pragma scop
  ...
  isl_map* rtile = isl_map_read_from_str(ctx, "[N, M] -> { [1, i1, i2, i3] -> [1, i1, o2, i3] : i1 >= 0 and 128i1 <= -1 + N and i2 >= 0 and i3 >= 0 and 128i3 <= -1 + N and o2 >= 1 + i2 and 128o2 <= -1 + M; [0, i1, i2, 0] -> [1, i1, o2, i2] : i1 >= 0 and 128i1 <= -1 + N and i2 >= 0 and 128i2 <= -1 + N and o2 >= 0 and 128o2 <= -1 + M }");
  isl_set* ii_set = isl_set_read_from_str(ctx, "[N, M] -> { [1, i1, i2, i3] : i1 >= 0 and 128i1 <= -1 + N and i2 >= 0 and 128i2 <= -1 + M and i3 >= 0 and 128i3 <= -1 + N; [0, i1, i2, 0] : i1 >= 0 and 128i1 <= -1 + N and i2 >= 0 and 128i2 <= -1 + N }");
